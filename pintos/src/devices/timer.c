...
struct semaphore blockedtimerSleep;
...
timer_init (void) 
{
  ...
  sema_init(&blockedtimerSleep,0);
  list_init(&waiter_list);
}
...
// function to compare two threads according to sleeping ticks
bool 
list_less_comp(const struct list_elem* a,const struct list_elem* b, void* aux UNUSED)
{
const int a_member = (list_entry(a, struct thread, elem)->sleepingTicks);
const int b_member = (list_entry(b, struct thread, elem)->sleepingTicks);
...
timer_sleep (int64_t ticks) 
{
  ...
  if(ticks>0){
    ...
    current_thread->sleepingTicks = start + ticks;          // set sleeping ticks of current thread
    enum intr_level previous_level = intr_disable();        // disable interrupt
    list_insert_ordered (&waiter_list, &current_thread->elem, &list_less_comp, NULL);   // add the current thread in its place in sorted waiter list
    thread_block ();   // block the current thread
    intr_set_level (previous_level);    // enable iterrupt
  }
}
