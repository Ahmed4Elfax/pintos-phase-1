...
/* If false (default), use realRound-robin scheduler.
...
{
  return realRound(realIntMultiplication(load_avg, 100));
}

/* Returns 100 times the current thread's recent_cpu value. */
int thread_get_recent_cpu(void)
{
  return realRound(realIntMultiplication(thread_current()->recent_cpu, 100));
}
...
running_thread(void)
{
  ...
  /* Copy the CPU's stack pointer into `esp', and then realRound that
